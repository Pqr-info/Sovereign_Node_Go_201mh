...
const SWEND_API_URL = process.env.SWEND_API_URL || 'https://pqr.info/REST/2.0';
const VAULT_ADDR = process.env.VAULT_ADDR || 'http://localhost:8200';
const VAULT_TOKEN = process.env.VAULT_TOKEN || '';
if (!process.env.VAULT_TOKEN) {
    console.warn("VAULT_TOKEN not set; Vault features disabled.");
}
...
export function activate(context: vscode.ExtensionContext) {
    console.log('SWEND Swarm is now active');
        ...
        tools: [
            {
                ...
                description: "Create a new forensic ticket in the SWEND Swarm fabric",
                inputSchema: {
                    type: "object",
                    properties: {
                        subject: { type: "string" },
        ...
        try {
            switch (name) {
                case "create_pqr_ticket":
                    const createResp = await axios.post(`${SWEND_API_URL}/ticket`, args);
                    return { content: [{ type: "text", text: JSON.stringify(createResp.data, null, 2) }] };

                case "query_pqr_memory":
                    const queryResp = await axios.get(`${SWEND_API_URL}/agent/${args?.agentID || 'antigravity'}/context?limit=${args?.limit || 10}`);
                    return { content: [{ type: "text", text: JSON.stringify(queryResp.data, null, 2) }] };

                case "link_pqr_tickets":
                    const linkResp = await axios.post(`${SWEND_API_URL}/ticket/${args?.parentID}/link/${args?.childID}`, {
                ...
                case "sync_github_issue":
                    ...
                    // Update PQR ticket
                    const syncResp = await axios.put(`${SWEND_API_URL}/ticket/${ticketID}`, {
                        Status: pqrStatus,
                ...
                case "create_healing_ticket":
                    const healingResp = await axios.post(`${SWEND_API_URL}/healing/ticket`, request.params.arguments);
                    return { content: [{ type: "text", text: JSON.stringify(healingResp.data, null, 2) }] };

                case "process_healing_iteration":
                    const iterResp = await axios.post(`${SWEND_API_URL}/healing/iterate/${request.params.arguments?.ticketID}`);
                    return { content: [{ type: "text", text: JSON.stringify(iterResp.data, null, 2) }] };

                case "record_healing_failure":
                    const failResp = await axios.post(`${SWEND_API_URL}/healing/failure`, request.params.arguments);
                    return { content: [{ type: "text", text: JSON.stringify(failResp.data, null, 2) }] };

                case "resolve_healing_ticket":
                    const resResp = await axios.post(`${SWEND_API_URL}/healing/resolve`, args);
                ...
                case "sovereign_override":
                    ...
                    const overrideResp = await axios.post(`${SWEND_API_URL}/emergency/bridge`, args, {
                        headers: { "X-Gemini-Key": geminiKey }
                    });
                    return { content: [{ type: "text", text: JSON.stringify(overrideResp.data, null, 2) }] };

                case "get_forensic_audit":
                    const auditResp = await axios.get(`${SWEND_API_URL}/tickets?limit=${args?.limit || 10}`);
    ...
    let emergencyRepair = vscode.commands.registerCommand('pqr.emergencyRepair', async () => {
        ...
        try {
            const geminiKey = process.env.GEMINI_API_KEY || "";
            const response = await axios.post(`${SWEND_API_URL}/emergency/bridge`, {
    ...
    let vaultSetup = vscode.commands.registerCommand('pqr.setupVault', async () => {
        ...
        if (auth === 'Authorize & Sweep') {
            try {
                ...
                await axios.post(`${VAULT_ADDR}/v1/secret/data/swend`, { data: secrets }, {
                    headers: { 'X-Vault-Token': VAULT_TOKEN }
                });

                vscode.window.showInformationMessage('✓ Secrets successfully swept into SWEND Vault. You can now safely remove the .env file.');
            } catch (error: any) {
...
async function updateTokenSentinel() {
    try {
        const response = await axios.get(`${SWEND_API_URL}/metrics/tokens`);
