...
This document provides the "Break-Glass" credentials and connection paths required to re-establish control over the SWEND Sovereign Node.
...
## 🛠️ Recovery Sequence
1. **Verify Infrastructure**: Run `docker ps` to ensure `db`, `vault`, `tunnel`, and `swend-server` are running.
