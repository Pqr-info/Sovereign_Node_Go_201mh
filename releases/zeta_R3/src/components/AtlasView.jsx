          ...
          <div key={agent.id} className="list-item" style={{ flexDirection: 'column', alignItems: 'flex-start' }}>
            ...
            <div className="card-row" style={{ width: '100%' }}>
              ...
              <span className="card-value" style={{ color: meshState.telemetry?.drift?.[agent.id] > 0 ? 'var(--color-yellow)' : 'inherit' }}>
                {meshState.telemetry?.drift?.[agent.id] !== undefined && !isNaN(Number(meshState.telemetry.drift[agent.id]))
                  ? Number(meshState.telemetry.drift[agent.id]).toFixed(2) 
                  : '0.00'}
              </span>
            </div>
          </div>
...
const NodeCard = ({ node }) => {
  ...
  const hashShort = (hash) => (hash && typeof hash === 'string') ? hash.substring(0, 10) + '...' : 'N/A';
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
          ...
          <div className="glass-card side-panel" style={{ flex: 1 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              {(Array.isArray(consensus) ? consensus : []).map((c, i) => (
                <div key={i} className="list-item">
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontWeight: 600 }}>{String(c.node || 'Unknown')}</span>
                    <span className={`tag ${getTagClass(c.decision)}`}>{String(c.decision || 'N/A')}</span>
                  </div>
                  <div className="card-row">
                    <span className="card-label">Confidence</span>
                    <span>{(!isNaN(Number(c.confidence)) ? (Number(c.confidence) * 100).toFixed(1) : '0.0')}%</span>
                  </div>
                  <div className="card-row">
                    <span className="card-label">Proposal ID</span>
                    <span style={{ fontFamily: 'monospace', fontSize: '0.75rem' }}>
                      {typeof c.proposal_id === 'string' ? c.proposal_id.substring(0,8) : 'N/A'}
                    </span>
                  </div>
                </div>
              ))}
              {(!Array.isArray(consensus) || consensus.length === 0) && <div style={{color:'var(--text-secondary)'}}>No active decisions.</div>}
            </div>
          </div>
          ...
          <div className="glass-card side-panel" style={{ flex: 1 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              {(Array.isArray(lineage) ? lineage : []).map((l, i) => (
                <div key={i} className="list-item">
                  <div style={{ fontWeight: 500 }}>{String(l.type || 'Unknown')}</div>
                  <div className="card-row">
                    <span className="card-label">{l.timestamp ? new Date(l.timestamp).toLocaleTimeString() : 'N/A'}</span>
                    {l.hash && <span className="card-value" style={{fontSize: '0.75rem'}}>{String(l.hash)}</span>}
                    {l.node && <span style={{fontSize: '0.875rem'}}>{String(l.node)}</span>}
                  </div>
                </div>
              ))}
              {(!Array.isArray(lineage) || lineage.length === 0) && <div style={{color:'var(--text-secondary)'}}>No events recorded.</div>}
            </div>
          </div>
