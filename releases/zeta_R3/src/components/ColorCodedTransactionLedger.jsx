...
export default function ColorCodedTransactionLedger() {
  ...
  const filteredTxs = transactions.map(tx => ({
    ...tx,
    tx_hash: String(tx.tx_hash || tx.tx_id || ''),
    from_addr: String(tx.from_addr || tx.from_address || ''),
    to_addr: String(tx.to_addr || tx.to_address || ''),
    type: String(tx.type || tx.category || 'TRANSFER'),
    env: String(tx.env || tx.category || '').replace(/[^a-zA-Z]/g, '').toUpperCase(),
    asset: typeof tx.asset === 'object' ? String(tx.asset.value || tx.asset.label || tx.currency || '') : String(tx.asset || tx.currency || ''),
    amount: typeof tx.amount === 'object' ? String(tx.amount.value || tx.amount.total || '0') : String(tx.amount || '0'),
    status: String(tx.status || 'UNKNOWN'),
    gas_fee: String(tx.gas_fee || tx.gas_used || '0.00')
  })).filter(tx => {
    const matchesEnv = activeFilter === 'ALL' || tx.env === activeFilter || tx.env.includes(activeFilter);
    const matchesSearch = 
