...
export class SovereignControlRoom {
  ...
  render() {
    ...
    el.innerHTML = `
      <div style="padding: 24px; color: #f8fafc; font-family: system-ui, -apple-system, sans-serif; background: #090d16; min-height: 100vh;">
        ...
        <!-- Navigation Tabs -->
        <div style="display: flex; gap: 8px; margin-bottom: 24px; border-bottom: 1px solid rgba(255,255,255,0.05); padding-bottom: 12px; flex-wrap: wrap;">
          ...
          ${this.renderTabBtn('copilot', '🧊 Copilot 7-Volley & Hotload')}
        </div>
  ...
  renderActivePanel() {
    switch (this.activeTab) {
      ...
      case 'copilot': return this.renderCopilotPanel();
  ...
  renderCopilotPanel() {
    return `
      <div style="display:flex; flex-direction:column; gap:20px;">
        <div style="background: rgba(15, 23, 42, 0.8); border: 1px solid #a855f7; border-radius: 14px; padding: 24px;">
          <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:16px;">
            <div>
              <h2 style="margin:0; color:#a855f7; font-size:20px;">👑 Copilot Master Architect Memory Graph Visualizer</h2>
              <div style="color:#94a3b8; font-size:13px; margin-top:4px;">
                Phase 21 Canon Spec: Cold Load 7-Volley (49 Slots, prefixVersion <code>v1.2.0</code>) & Single-Volley Hotload Delta
              </div>
            </div>
            <span style="background:rgba(168,85,247,0.2); border:1px solid #a855f7; color:#e9d5ff; font-size:12px; font-weight:700; padding:6px 12px; border-radius:20px;">
              CANON COHERENT
            </span>
          </div>
          ...
          <div style="display:flex; gap:12px; margin-bottom:20px; flex-wrap:wrap;">
            <button id="btnColdLoadVolley1" style="background:#0284c7; border:none; color:#fff; padding:10px 16px; border-radius:8px; cursor:pointer; font-weight:600;">
              📥 Fetch Volley 1/7
            </button>
            <button id="btnColdLoadVolley7" style="background:#7c3aed; border:none; color:#fff; padding:10px 16px; border-radius:8px; cursor:pointer; font-weight:600;">
              🧊 Fetch Volley 7/7 (NOW Cube)
            </button>
            <button id="btnHotloadDelta" style="background:#059669; border:none; color:#fff; padding:10px 16px; border-radius:8px; cursor:pointer; font-weight:600;">
              🔥 Fetch Hotload Delta
            </button>
            <button id="btnResetContextSession" style="background:#dc2626; border:none; color:#fff; padding:10px 16px; border-radius:8px; cursor:pointer; font-weight:600;">
              🔄 Reset Context Session
            </button>
          </div>

          <!-- Console Output Box -->
          <div style="background:#030712; border:1px solid rgba(255,255,255,0.1); border-radius:10px; padding:16px;">
            <div style="color:#06b6d4; font-size:12px; font-weight:700; margin-bottom:8px; display:flex; justify-content:space-between;">
              <span>CANON RESPONSE PAYLOAD LOG</span>
              <span id="copilotLogStatus" style="color:#10b981;">Ready</span>
            </div>
            <pre id="copilotJsonOutput" style="margin:0; color:#f1f5f9; font-family:monospace; font-size:12px; white-space:pre-wrap; max-height:400px; overflow-y:auto;">
Click a button above to inspect live 7-Volley Cold Load or Hotload Delta JSON payload.
            </pre>
          </div>
        </div>
      </div>
  ...
  bindEvents() {
    ...
    // Copilot Memory Graph Buttons
    const outPre = document.getElementById('copilotJsonOutput');
    const outStatus = document.getElementById('copilotLogStatus');

    const updateCopilotLog = (statusText, data) => {
      if (outStatus) outStatus.textContent = statusText;
      if (outPre) outPre.textContent = JSON.stringify(data, null, 2);
    };

    const v1 = document.getElementById('btnColdLoadVolley1');
    if (v1) {
      v1.addEventListener('click', async () => {
        if (outStatus) outStatus.textContent = 'Fetching Volley 1/7...';
        const res = await fetch(`${this.apiBase}/api/memorygraph/coldload/1?sessionId=controlroom_ui`).then(r => r.json()).catch(e => ({ error: e.message }));
        updateCopilotLog('Volley 1/7 Received', res);
      });
    }

    const v7 = document.getElementById('btnColdLoadVolley7');
    if (v7) {
      v7.addEventListener('click', async () => {
        if (outStatus) outStatus.textContent = 'Fetching Volley 7/7 (NOW Cube)...';
        const res = await fetch(`${this.apiBase}/api/memorygraph/coldload/7?sessionId=controlroom_ui`).then(r => r.json()).catch(e => ({ error: e.message }));
        updateCopilotLog('Volley 7/7 (NOW Cube) Received', res);
      });
    }

    const hot = document.getElementById('btnHotloadDelta');
    if (hot) {
      hot.addEventListener('click', async () => {
        if (outStatus) outStatus.textContent = 'Fetching Hotload Delta...';
        const res = await fetch(`${this.apiBase}/api/memorygraph/hotload?sessionId=controlroom_ui`).then(r => r.json()).catch(e => ({ error: e.message }));
        updateCopilotLog('Hotload Delta Received', res);
      });
    }

    const reset = document.getElementById('btnResetContextSession');
    if (reset) {
      reset.addEventListener('click', async () => {
        if (outStatus) outStatus.textContent = 'Resetting Session Warmth...';
        const res = await fetch(`${this.apiBase}/api/memorygraph/reset`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ sessionId: 'controlroom_ui' })
        }).then(r => r.json()).catch(e => ({ error: e.message }));
        updateCopilotLog('Session Reset Complete', res);
      });
    }
  }
}
