...
const CRITICAL_SERVICES = [
  { name: 'HashiCorp Vault Emulator', port: 8200, command: 'node src/engine/vault_emulator.js', ping: '/v1/sys/health' },
  { name: 'Zeta Master Compute', port: 4052, command: 'node zeta_l7_service.js', ping: '/antigravity/poll' },
  { name: 'Atlas UI Dev Server', port: 9080, command: 'npm.cmd run dev', ping: '/' },
  { name: 'Sovereign-MGSH-MCP Engine', port: 11112, command: 'C:\\Users\\theal\\gsd-venv\\Scripts\\python.exe C:\\Users\\theal\\SWEND-MESH\\SUBSTRATE\\grpc_node\\grpc_server.py', isTcp: true }
];

const processes = {};
const processedLogLines = new Set();

function pingService(port, path, method = 'GET', headers = {}, isTcp = false) {
  return new Promise((resolve) => {
    if (isTcp) {
      ...
      socket.setTimeout(2000, () => { socket.destroy(); resolve({ ok: false }); });
      return;
    }
...
function startService(svc) {
  ...
  proc.on('error', (err) => console.error(`[Ouroboros Sentinel] Failed to spawn ${svc.name}:`, err.message));
  proc.unref();
...
export async function runSelfHealingAuditMatrix() {
  ...
  for (const svc of CRITICAL_SERVICES) {
    const ping = await pingService(svc.port, svc.ping, 'GET', {}, svc.isTcp);
    if (!ping.ok) {
      ...
      auditReport.auto_healed_actions.push({
        ...
        how: `Spawned detached process '${svc.command}'`,
...
async function main() {
  ...
  setInterval(async () => {
    try {
      ...
      console.log(`\n[Ouroboros Sentinel] --- EXECUTION #${execCount} | UPTIME: ${uptimeSecs}s ---`);
