...
export default function MEVArbitragePanel({ onClose }) {
  ...
  const [networkKey, setNetworkKey] = useState('BASE_SEPOLIA');
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
            ...
            <select
              ...
              <option value="BASE_SEPOLIA">Base Sepolia Testnet (Zero Cost - ARBITRAGE TESTNET)</option>
        ...
        <div style={{ flex: 1, padding: '1rem', overflowY: 'auto', background: 'rgba(0,0,0,0.1)' }}>
          {selectedRoute ? (
            <div>
              ...
              <div style={{ fontSize: '0.8rem', fontWeight: 600, color: 'var(--text-secondary)', marginBottom: '0.4rem' }}>Swaps Pipeline ({selectedRoute.pools?.length || 0} Legs):</div>
              ...
              {simResult && (
                <div style={{ background: 'var(--bg-1)', padding: '0.75rem', borderRadius: '6px', border: `1px solid ${simResult.substrate_batch_ready || simResult.ok ? '#10b981' : '#ef4444'}`, marginTop: '1rem' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.3rem' }}>
                    <span style={{ fontSize: '0.8rem', fontWeight: 700, color: simResult.substrate_batch_ready || simResult.ok ? '#10b981' : '#ef4444' }}>
                      Auditor Shadow Trial: {simResult.auditor_status || (simResult.ok ? 'SHADOW_PASSED' : 'FAILED')}
                    </span>

                    {(simResult.substrate_batch_ready || simResult.ok) && (
                      <button
                  ...
                  <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginBottom: '0.4rem' }}>
                    Shadow Latency: {simResult.shadow_latency_ms || simResult.execution_latency_ms || 0}ms | Target Network: {networkKey}
                  </div>
                  <pre style={{ background: '#09090b', padding: '0.4rem', borderRadius: '4px', fontSize: '0.65rem', color: simResult.substrate_batch_ready || simResult.ok ? '#34d399' : '#f87171', margin: 0 }}>
